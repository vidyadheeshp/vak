...
{perf_rows()}
  ...
  during the optimisation work. They record what changed, and cannot be
  regenerated — the &ldquo;before&rdquo; code is gone. Current performance can
  be: <code>python bench/benchmark.py</code> measures Vāk against C, CPython,
  Node and PHP, and says plainly which of them these workloads are too small to
  measure rather than printing a number that looks meaningful.</figcaption>
</div>
